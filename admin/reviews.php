    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-700 font-bold">Manage Reviews</h1>
            </div>
        ...
        <?php if (!isset($_GET['q'])) : ?>
            <div class="grid grid-cols-12 gap-6">
                <div class="col-span-full">
                    <div class="flex pb-3 border-b-2">
                        <a href="./reviews.php?type=text" class="font-bold text-xs uppercase <?= $type == 'text' ? 'text-indigo-500' : 'text-indigo-300' ?>">Text</a>
                        <a href="./reviews.php?type=video" class="ml-3 font-bold text-xs uppercase <?= $type == 'video' ? 'text-indigo-500' : 'text-indigo-300' ?>">Video</a>
                    </div>
                </div>
                            ...
                            <div class="flex flex-col h-full">
                                <div class="grow p-5">
                                    <div class="flex justify-between items-start">
                                        <header>
                                            <div class="flex mb-2">
                                                <a class="relative inline-flex items-start mr-5" href="">
                                                ...
                                                <div class="mt-1 pr-1"><a class="inline-flex text-gray-800 hover:text-gray-900" href="">
                                                    ...
                                                    <div class="flex mt-3 items-center space-x-1">
                                                        ...
                                                        <?php for ($i = 0; $i < intval($review['rating']); $i++) : ?>
                                                            <button><span class="sr-only">1 star</span> <svg class="w-4 h-4 fill-current text-yellow-500" viewBox="0 0 16 16">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <?php else : ?>
                                            <div class="text-sm">
                                                <video controls src="../reviews/<?= $review['file'] ?>"></video>
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <?php if ($review['is_featured']) : ?>
                                            <form method="post" action="./handler/review_handler.php" class="block flex-1 text-center text-sm text-red-600 hover:text-red-800 font-medium px-3 py-4 group">
                                                <button name="unfeature" value="<?= $review['review_id']; ?>" class="flex items-center justify-center">
                                                    <svg class="w-4 mr-2 h-4 fill-current text-red-500 shrink-0" viewBox="0 0 16 16">
                                        ...
                                        <?php else : ?>
                                            <form method="post" action="./handler/review_handler.php" class="block flex-1 text-center text-sm text-gray-600 hover:text-gray-800 font-medium px-3 py-4 group">
                                                <button name="feature" value="<?= $review['review_id']; ?>" class="flex items-center text-yellow-600 justify-center">
                                                    <svg class="w-4 mr-2 h-4 fill-current text-yellow-500 shrink-0" viewBox="0 0 16 16">
                        ...
                        <div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white shadow-lg rounded-sm border border-gray-200">
                            <div class="flex flex-col h-full">
                                <div class="grow p-5">
                                    <div class="flex justify-between items-start">
                                        <header>
                                            <div class="flex mb-2"><a class="relative inline-flex items-start mr-5" href="">
                                                    <div class="absolute top-0 right-0 -mr-2 bg-white rounded-full shadow" aria-hidden="true"><svg class="w-8 h-8 fill-current text-yellow-500" viewBox="0 0 32 32">
                                                            <path d="M21 14.077a.75.75 0 01-.75-.75 1.5 1.5 0 00-1.5-1.5.75.75 0 110-1.5 1.5 1.5 0 001.5-1.5.75.75 0 111.5 0 1.5 1.5 0 001.5 1.5.75.75 0 010 1.5 1.5 1.5 0 00-1.5 1.5.75.75 0 01-.75.75zM14 24.077a1 1 0 01-1-1 4 4 0 00-4-4 1 1 0 110-2 4 4 0 004-4 1 1 0 012 0 4 4 0 004 4 1 1 0 010 2 4 4 0 00-4 4 1 1 0 01-1 1z" />
                                                        </svg></div><img class="rounded-full" src="images/user-64-01.jpg" width="64" height="64" alt="User 01" />
                                                </a>
                                                <div class="mt-1 pr-1"><a class="inline-flex text-gray-800 hover:text-gray-900" href="">
                                                        <h2 class="text-xl leading-snug justify-center font-semibold">
                                                            Dominik McNeail</h2>
                                                    </a>
                                                    <div class="flex items-center"><span class="text-sm font-medium text-gray-400 -mt-0.5 mr-1">-&gt;</span>
                                                        <span>🇮🇹</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="relative inline-flex shrink-0" x-data="{ open: false }"><button class="text-gray-400 hover:text-gray-500 rounded-full" :class="{ 'bg-gray-100 text-gray-500': open }" aria-haspopup="true" @click.prevent="open = !open" :aria-expanded="open"><span class="sr-only">Menu</span> <svg class="w-8 h-8 fill-current" viewBox="0 0 32 32">
                                            ...
                                            <div class="origin-top-right z-10 absolute top-full right-0 min-w-36 bg-white border border-gray-200 py-1.5 rounded shadow-lg overflow-hidden mt-1" @click.outside="open = false" @keydown.escape.window="open = false" x-show="open" x-transition:enter="transition ease-out duration-200 transform" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak>
                                                <ul>
                                                    <li><a class="font-medium text-sm text-gray-600 hover:text-gray-800 flex py-1 px-3" href="#" @click="open = false" @focus="open = true" @focusout="open = false">Option 1</a></li>
                                                    <li><a class="font-medium text-sm text-gray-600 hover:text-gray-800 flex py-1 px-3" href="#" @click="open = false" @focus="open = true" @focusout="open = false">Option 2</a></li>
                                                    <li><a class="font-medium text-sm text-red-500 hover:text-red-600 flex py-1 px-3" href="#" @click="open = false" @focus="open = true" @focusout="open = false">Remove</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                ...
                                <div class="border-t border-gray-200">
                                    <div class="flex divide-x divide-gray-200r"><a class="block flex-1 text-center text-sm text-indigo-500 hover:text-indigo-600 font-medium px-3 py-4" href="messages.php">
                                            <div class="flex items-center justify-center"><svg class="w-4 h-4 fill-current shrink-0 mr-2" viewBox="0 0 16 16">
                                                    <path d="M8 0C3.6 0 0 3.1 0 7s3.6 7 8 7h.6l5.4 2v-4.4c1.2-1.2 2-2.8 2-4.6 0-3.9-3.6-7-8-7zm4 10.8v2.3L8.9 12H8c-3.3 0-6-2.2-6-5s2.7-5 6-5 6 2.2 6 5c0 2.2-2 3.8-2 3.8z" />
                                                </svg> <span>Send Email</span></div>
                                        </a><a class="block flex-1 text-center text-sm text-gray-600 hover:text-gray-800 font-medium px-3 py-4 group" href="./settings">
                                            <div class="flex items-center justify-center"><svg class="w-4 h-4 fill-current text-gray-400 group-hover:text-gray-500 shrink-0 mr-2" viewBox="0 0 16 16">
                        ...
                        <div class="flex justify-center">
                            <nav class="flex" role="navigation" aria-label="Navigation">
                                ...
                                <ul class="inline-flex text-sm font-medium -space-x-px shadow-sm">
                                    ...
                                    <li><a class="inline-flex items-center justify-center leading-5 px-3.5 py-2 bg-white hover:bg-indigo-500 border border-gray-200 text-gray-600 hover:text-white" href="#">2</a></li>
                                    <li><a class="inline-flex items-center justify-center leading-5 px-3.5 py-2 bg-white hover:bg-indigo-500 border border-gray-200 text-gray-600 hover:text-white" href="#">3</a></li>
                                    ...
                                    <li><a class="inline-flex items-center justify-center rounded-r leading-5 px-3.5 py-2 bg-white hover:bg-indigo-500 border border-gray-200 text-gray-600 hover:text-white" href="#">9</a></li>
                                </ul>
                                <div class="ml-2">
                                    <a href="#" class="inline-flex items-center justify-center rounded leading-5 px-2.5 py-2 bg-white hover:bg-indigo-500 border border-gray-200 text-gray-600 hover:text-white shadow-sm"><span class="sr-only">Next</span><wbr />
                                        <svg class="h-4 w-4 fill-current" viewBox="0 0 16 16">
